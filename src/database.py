...
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://dao_user:dao_password@localhost:5432/dao_db")
    ...
    try:
        ...
        with SessionLocal() as session:
            session.execute(text("""
                CREATE TABLE IF NOT EXISTS dao_memories (
            ...
            """))
            session.commit()
            logger.info("Ensured 'dao_memories' table exists.")
        ...
        with SessionLocal() as session:
            ...
            """))
            session.commit()
            logger.info("Ensured 'cognitive_state_history' table exists with loss columns.")
