...
class SelfPromptingModule(nn.Module):
    """
    A module responsible for dynamically generating internal prompts for dao's self-reflection.
    It simulates dao's internal inquiry, allowing it to autonomously explore concepts
    ...
    def generate_prompt(self, current_internal_states: dict) -> str:
        """
        Dynamically generates an internal prompt based on dao's current internal states.
        
        Args:
            current_internal_states (dict): A dictionary containing dao's
