...
import { useTrainingStatus } from './context/TrainingContext';
...
export default function Home() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const data: DaoResponse = await res.json();
      const daoResponse: ChatMessage = { sender: 'dao', text: data.response || "Dao couldn't generate a response." };
      ...
      const daoDebugInfo: ChatMessage = { sender: 'dao', text: debugInfoText };
      setMessages((prevMessages) => [...prevMessages, daoResponse, daoDebugInfo]);

    } catch (error) {
      console.error('Error generating response:', error);
      setMessages((prevMessages) => [
        ...prevMessages,
        { sender: 'dao', text: `Error: Failed to connect to dao's core or functionality paused. (${error instanceof Error ? error.message : String(error)})` },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div style={{ padding: '1rem', width: '100%' }}>
      <h1 style={{ marginBottom: '1.5rem', textAlign: 'center' }}>dao - The Living Dynamo</h1>
        ...
        <div style={{ flexGrow: 1, overflowY: 'auto', maxHeight: '60vh', padding: '1.5rem' }}>
          {messages.length === 0 ? (
            <p style={{ textAlign: 'center', color: 'var(--foreground-subtle)' }}>
              {isChatDisabled ? "Chat is paused during training." : "Start a conversation with dao..."}
            </p>
          ) : (
            messages.map((msg, index) => (
              <div key={index} style={{ display: 'flex', justifyContent: msg.sender === 'user' ? 'flex-end' : 'flex-start', marginBottom: '0.8rem' }}>
                <div style={{
                  backgroundColor: msg.sender === 'user' ? 'var(--chat-bubble-user-bg)' : 'var(--chat-bubble-dao-bg)',
                  border: msg.sender === 'dao' ? `1px solid var(--card-border)` : 'none',
                  borderRadius: '12px', padding: '0.7rem 1rem', maxWidth: '80%', wordWrap: 'break-word',
                  fontSize: msg.sender === 'dao' && msg.text.startsWith('(') ? '0.75rem' : '1rem',
                  color: msg.sender === 'dao' && msg.text.startsWith('(') ? 'var(--foreground-subtle)' : 'var(--foreground)'
                }}>
                  <strong>{msg.sender === 'user' ? 'You' : 'dao'}:</strong> {msg.text}
                </div>
              </div>
            ))
          )}
