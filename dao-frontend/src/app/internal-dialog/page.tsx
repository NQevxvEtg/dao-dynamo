...
export default function InternalDialogPage() {
  ...
  return (
    <main style={{
      ...
      <h1 style={{ marginBottom: '1.5rem' }}>dao&apos;s Internal Dialog</h1>
      <p style={{ marginBottom: '2rem', color: 'var(--foreground-subtle)', textAlign: 'center' }}>
        Observe dao&apos;s continuous internal reflections and mental state changes.
      </p>
      ...
      <div style={{
        ...
        <div style={{ overflowY: 'auto', maxHeight: '70vh', paddingRight: '1rem' }}>
          {isLoading && thoughts.length === 0 ? (
            <p style={{ textAlign: 'center', color: 'var(--foreground-subtle)' }}>Waking dao&apos;s mind... Please wait.</p>
            ...
            thoughts.map((thought, index) => (
              <div key={index} style={{
                backgroundColor: 'var(--chat-bubble-dao-bg)', // Using a consistent bubble color
                borderRadius: '10px',
                padding: '1rem',
                marginBottom: '1rem',
