...
class MambaInnerFn(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        # --- FIX 1 ---
        conv1d_out = torch.zeros_like(x, dtype=torch.float32)
    ...
    def backward(ctx, dout):
        ...
        if ctx.checkpoint_lvl == 1:
            x_bwd = x.to(torch.float32)
            # --- FIX 2 ---
            conv1d_out = torch.zeros_like(x_bwd, dtype=torch.float32)
            causal_conv1d_cuda.causal_conv1d_fwd(
                x_bwd, conv1d_weight, conv1d_bias, None, None, conv1d_out, None, True
            )
            delta = rearrange(delta_proj_weight @ x_dbl[:, :delta_rank].t(),
                              "d (b l) -> b d l", l = L)

        # --- FIX 3 ---
        dxz = torch.zeros_like(xz)
        ...
        dx, dconv1d_weight, dconv1d_bias = causal_conv1d_cuda.causal_conv1d_bwd(
            x_bwd, conv1d_weight, conv1d_bias, dconv1d_out, None, dx, True
        )
